...
export default {
    encode: async (_id, rol, user, nombre, unidad) => {
        const token = jwt.sign({_id:_id, rol:rol, user:user, nombre:nombre, unidad:unidad},'clavesecretaparagenerartoken',{expiresIn: '1d'});
        return token;
    },
    decode: async (token) => {
        try {
